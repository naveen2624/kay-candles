        ...
        <div className="max-w-2xl mx-auto text-center">
          ...
          <h1 className="font-display text-4xl font-light text-blush-900 mb-6">
            {query ? (
              <>
                Results for &ldquo;
                <em className="italic text-blush-400">{query}</em>&rdquo;
              </>
            ) : (
          ...
          <SearchBar
            size="lg"
            placeholder="Search candles, flowers, gifts…"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {initialResults.length === 0 && query ? (
          <div className="text-center py-20">
            ...
            <h2 className="font-accent text-xl text-blush-600 mb-2">
              No results found
            </h2>
          ...
          <div className="space-y-14">
            <p className="font-body text-sm text-blush-500">
              ...
              result{initialResults.length !== 1 ? "s" : ""}
              {query && (
                <>
                  {" "}
                  for &ldquo;<strong className="text-blush-700">{query}</strong>
                  &rdquo;
                </>
              )}
            </p>
                ...
                {candles.length > 0 && (
                  <div>
                    <h2 className="font-display text-3xl font-light text-blush-900 mb-6">
                      Candles{" "}
                      <span className="text-blush-300 text-xl">
                        ({candles.length})
                      </span>
                    </h2>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            {initialResults.length < 4 && suggestions.length > 0 && (
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                  {suggestions
                    .filter((s) => !initialResults.find((r) => r.id === s.id))
                    .slice(0, 4)
                    .map((p) => (
                      <ProductCard key={p.id} product={p} />
                    ))}
